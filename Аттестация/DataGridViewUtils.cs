...
namespace Аттестация
{
    public class DataGridViewUtils
    {
        ...
        public static List<LogicLibrary.Student> GridToStudents(DataGridView dgv)
        {
            List<LogicLibrary.Student> students = new List<LogicLibrary.Student>();
            int n = dgv.RowCount;
            for (int i = 0; i < n; i++)
            {
                if (dgv[0, i].Value != null)
                {
                    string fio = Convert.ToString(dgv[0, i].Value);
                    DateTime year = Convert.ToDateTime(dgv[1, i].Value);
                    string[] medbs = Convert.ToString(dgv[2, i].Value).Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    int[] medb = new int[medbs.Length];
                    for (int j = 0; j < medb.Length; j++)
                    {
                        medb[j] = Convert.ToInt32(medbs[j]);
                    }
                    byte kurs = Convert.ToByte(dgv[3, i].Value);
                    byte group = Convert.ToByte(dgv[4, i].Value);
                    LogicLibrary.Student st = new LogicLibrary.Student() { FIO = fio, Year = year, MedB = medb, Kurs = kurs, Group = group };
                    students.Add(st);
                }
            }
            return students;
        ...
        public static void StudentToGrid(List<LogicLibrary.Student> students, DataGridView dgv)
        {
            int n = students.Count;
            if (dgv.RowCount < n + 1)
            {
                int y = n - dgv.RowCount + 1;
                for (int t = 0; t < y; t++)
                    dgv.Rows.Add();
            }
            int i = 0;
            foreach (LogicLibrary.Student std in students)
            {
                //if (i != 11)
                //dgv.Rows.Add();
                dgv[0, i].Value = std.FIO;
                dgv[1, i].Value = std.Year;
                for (int j = 0; j < std.MedB.Length; j++)
                {
                    dgv[2, i].Value = dgv[2, i].Value + Convert.ToString(std.MedB[j]) + " ";
                }
                dgv[3, i].Value = std.Kurs;
                dgv[4, i].Value = std.Group;
        ...
        //public static void StudentToGrid(List<List<LogicLibrary.Student>> students, DataGridView dgv)
        //{
        //    int i = 0;
        //    foreach (List<LogicLibrary.Student> liststd in students)
        //    {
        //        dgv.Rows.Add();
        //        dgv[0, i].Value = "Курс "+liststd[0].Kurs;
        //        i++;
        //        foreach (LogicLibrary.Student std in liststd)
        //        {
        //            dgv.Rows.Add();
        //            dgv[0, i].Value = std.FIO;
        //            dgv[1, i].Value = std.Year;
        ...
        //    }
        //}
        public static void KursesToGrid(List<LogicLibrary.Kurs> kurses, DataGridView dgv)
        {
            int i = 0;
            foreach (LogicLibrary.Kurs kurs in kurses)
            {
                dgv.Rows.Add();
                dgv[0, i].Value = "Курс " + kurs.NumberKurs;
                i++;
                foreach (LogicLibrary.Student std in kurs.Students)
                {
                    dgv.Rows.Add();
                    dgv[0, i].Value = std.FIO;
                    dgv[1, i].Value = std.Year;
